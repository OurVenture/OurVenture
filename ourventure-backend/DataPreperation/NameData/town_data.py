...
def unzip_cities():
    ...
    dir_path = "ourventure-backend/DataPreperation/DataCollections"
    if not os.path.exists(f"{dir_path}/cities5000.txt"):
        # Checks that the ZIP file exists (not ignored by git)
        if os.path.exists("ourventure-backend/DataPreperation/cities5000.zip"):
            # print("Extracting zip file")
            try: 
                with ZipFile("ourventure-backend/DataPreperation/cities5000.zip", "r") as zp:
                    zp.extractall(path=dir_path)
